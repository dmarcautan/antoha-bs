...
class MilestonesClaimByLkPrtctrd(BSMessageReader):
    ...
    def process(self):
        if self.Road==10:
            if self.Season in [2]:
                if self.Level in [2,14,22,36,44,52,62]: #Гемы
                ...
                elif self.Level in [6,18,32,48,56,67]: #золото
                ...
                elif self.Level in [1,4,8,12,16,21,24,28,33,38,41,43,46,54,58,63]: #маленькие
                    Supply(self.client, self.player, "BOXLkPrtctrd", {"BoxDID": 10, "Road": self.Road, "Season": self.Season, "Level": self.Level}).send()
                elif self.Level in [0,3,5,7,9,11,13,15,17,19,23,25,27,29,31,35,37,39,47,49,53,57,59,61,66,70]: #большие
                    Supply(self.client, self.player, "BOXLkPrtctrd", {"BoxDID": 12, "Road": self.Road, "Season": self.Season, "Level": self.Level}).send()
                elif self.Level in [10,20,30,40,45,51,55,60,65,68]: #мега
                    Supply(self.client, self.player, "BOXLkPrtctrd", {"BoxDID": 11, "Road": self.Road, "Season": self.Season, "Level": self.Level}).send()
                elif self.Level in [26,34,42,50,64,69]: #очки силы
                    try:
                        self.Character
                        amount = Help().GetAmountOfLevel(self.Level, [26,34,42,50,64,69], [50,50,75,100,100,500])
                        ...
                        boxalternate = Help().GetAmountOfLevel(self.Level, [26,34,42,50,64,69], [10,10,10,12,12,11])
                        Supply(self.client, self.player, "BOXLkPrtctrd", {"BoxDID": boxalternate, "Road": self.Road, "Season": self.Season, "Level": self.Level}).send()
        elif self.Road==9:
            if self.Season in [2]:
                if self.Level in [2,4,8,10,14,23,34,45]: #золото бп+
                    amount = Help().GetAmountOfLevel(self.Level+1, [3,5,9,11,15,24,35,45], [100,100,100,100,200,200,200,300])
                    Supply(self.client, self.player, "BPLkPrtctrd", {"Type":7,"Amount": amount, "Road": self.Road, "Season": self.Season, "Level": self.Level}).send()
                elif self.Level in [3,7,11,13,17,20,25,26,31,32,35,39,43]: #биг бокс бп+
                    Supply(self.client, self.player, "BOXLkPrtctrd", {"BoxDID": 12, "Road": self.Road, "Season": self.Season, "Level": self.Level}).send()
                elif self.Level in [1,9,19,29,41]: #мега бп+
                    Supply(self.client, self.player, "BOXLkPrtctrd", {"BoxDID": 11, "Road": self.Road, "Season": self.Season, "Level": self.Level}).send()
                elif self.Level in [5]:
                    boxalternate = 12
                    Supply(self.client, self.player, "BOXLkPrtctrd", {"BoxDID": boxalternate, "Road": self.Road, "Season": self.Season, "Level": self.Level}).send()
                elif self.Level in [15]:
                    brawler = Help().GetAmountOfLevel(self.Season+1, [3], [39])
                    ...
                    brawler = Help().GetAmountOfLevel(self.Season+1, [3], [13]) #Бравлер Айди (брок)
                    skin = Help().GetAmountOfLevel(self.Season+1, [3], [214]) #Скин айди (суперрейнджер)
                    if self.player.UnlockedSkins[str(skin)] == 0:
                        ...
                        Supply(self.client, self.player, "BOXLkPrtctrd", {"BoxDID": 12, "Road": self.Road, "Season": self.Season, "Level": self.Level}).send()
                elif self.Level in [49]:
                    brawler = Help().GetAmountOfLevel(self.Season+1, [3], [39]) #Бравлер Айди (вольт)
                    skin = Help().GetAmountOfLevel(self.Season+1, [3], [212]) #Скин айди (паладин)
                    if self.player.UnlockedSkins[str(skin)] == 0:
                        amount = 1
                        self.Skin = [29,skin]
                        self.Character = [16,brawler]
                        ...
                        Supply(self.client, self.player, "BOXLkPrtctrd", {"BoxDID": 12, "Road": self.Road, "Season": self.Season, "Level": self.Level}).send()
                elif self.Level in [6,12,22,28,37,47]: #очки силы бп+
                    try:
                        self.Character
                        amount = Help().GetAmountOfLevel(self.Level, [6,12,22,28,37,47], [50,100,100,200,200,500])
                        Supply(self.client, self.player, "BPLkPrtctrd", {"Character": self.Character, "Type":6,"Amount": amount, "Road": self.Road, "Season": self.Season, "Level": self.Level}).send()
                    except:
                        boxalternate = Help().GetAmountOfLevel(self.Level, [6,12,22,28,37,47], [10,12,12,12,12,11])
                        Supply(self.client, self.player, "BOXLkPrtctrd", {"BoxDID": boxalternate, "Road": self.Road, "Season": self.Season, "Level": self.Level}).send()
                elif self.Level in [16,18,21,24,27,30,33,35,36,38,40,42,44,46,48]: #пины литл бокс
                    boxalternate = 10
                    Supply(self.client, self.player, "BOXLkPrtctrd", {"BoxDID": boxalternate, "Road": self.Road, "Season": self.Season, "Level": self.Level}).send()

        elif self.Road == 6: #тропроад
        ...
        import json
        if self.Road in [9] and self.Season in [2]:
            try:
        ...
        if self.Road in [10] and self.Season in [2]:
            try:
