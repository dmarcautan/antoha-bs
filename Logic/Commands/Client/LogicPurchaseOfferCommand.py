...
class LogicPurchaseOfferCommand(BSMessageReader):
    ...
    def process(self):
        ...
        if id1 == 8 or id2 == 8 or id3 == 8:
            for i, (brawler_id, multiplier) in enumerate(zip([brawler1, brawler2, brawler3], [multi1, multi2, multi3])):
                if brawler_id != 0:
                    current_points = self.player.brawlerPoints.get(str(brawler_id), 0)
                    new_points = current_points + multiplier
                    self.player.brawlerPoints[str(brawler_id)] = new_points
                    dbupd = 'brawlerPoints'
                    DataBase.replaceValue(self, dbupd, self.player.brawlerPoints)
        ...
        if id1 in [6, 10, 14]:
            ...
            if offers[self.offer_index]['ShopType']==0:
                ...
                self.player.starpoints -= offers[self.offer_index]['Cost']
                dbupd = 'starpoints'
                DataBase.replaceValue(self, dbupd, self.player.starpoints)