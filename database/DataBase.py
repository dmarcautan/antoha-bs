...
class DataBase:
    def loadAccount(self):
        ...
        try:
            ...
            LoginFailedMessage(self.client, self.player, "ERROR: 0008 - Очистите данные о приложении!").send()
        ...
        if user_data:
            ...
            self.player.brawlers_spg_unlock = brawlerData["brawlersSpgUnlock"]
    ...
    def createAccount(self):
        if os.path.exists("database/Player/plr.db"):
            ...
            var = (self.player.token, self.player.low_id, self.player.name, self.player.trophies, self.player.gold, self.player.gems, self.player.starpoints, self.player.tickets, self.player.Troproad, self.player.profile_icon, self.player.name_color, self.player.club_low_id, self.player.club_role, json.dumps({"highest_trophies": self.player.highest_trophies, "brawlersTrophies": self.player.brawlers_trophies, "UnlockedBrawlers": self.player.UnlockedBrawlers, "UnlockedSkins": self.player.UnlockedSkins, "brawlerPowerLevel": self.player.brawlerPowerLevel, "brawlerPoints": self.player.brawlerPoints, "brawlersSpgUnlock": self.player.brawlers_spg_unlock}), self.player.brawler_id, self.player.skin_id, self.player.room_id, self.player.box, self.player.bigbox, self.player.online, self.player.vip, self.player.player_experience, json.dumps([]), self.player.ccc, self.player.trioWINS, self.player.sdWINS, self.player.theme, self.player.BPTOKEN, self.player.BPXP, json_quests, jsonFBP, jsonBBP, self.player.notifRead, self.player.notifRead2, self.player.ip_address, self.player.creation_date, self.player.Region)
    ...
    def getSuggestions(self):
        self.conn = sql.connect("database/Player/plr.db")
        self.cur = self.conn.cursor()
        self.cur.execute("SELECT lowID,name,trophies,profile_icon,name_color,friends FROM plrs ORDER BY trophies DESC LIMIT 50")
    ...
    def GetLeaderboardByBrawler(self, ID):
        self.conn = sql.connect("database/Player/plr.db")
        self.cur = self.conn.cursor()
        self.cur.execute("SELECT lowID, name, brawlerData, profile_icon, name_color, vip FROM plrs")
        fetch = self.cur.fetchall()
        self.conn.close()
        fetch.sort(key=lambda plr: json.loads(plr[2])['brawlersTrophies'].get(str(ID), 0), reverse=True)
        return fetch
    ...
    def replaceClubValue(self, target, inf1, inf2, inf3, inf4, inf5, inf6):
        self.conn = sql.connect("database/Club/clubs.db")
        self.con = sql.connect("database/Club/chats.db")
        self.cur= self.conn.cursor()
        self.chat = self.con.cursor()
        ...
        if 1==1:
            ...
            self.cur.execute(f"UPDATE clubs SET trophies={inf6} WHERE clubID={self.player.club_low_id}")
